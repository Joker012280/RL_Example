...
## Behavior Model load if it's exists
behavior_path = "./idea_result/bc_Walker2d.pth"

if behavior_path is not None :
    temp = torch.load(behavior_path)
    agent.behavior_network.load_state_dict(temp['model_state_dict'],strict = False)
    agent.behavior_network.eval()
    agent.behavior_train = False
    print("End Loading")
else :
    raise e
