...
class CQL(nn.Module):
    ...
    def _compute_policy_values(self, obs_pi, obs_q):
        with torch.no_grad():
            _,action_dist = self.actor_network.evaluate(obs_pi)
    ...
    def train_net(self):
        ...
        action_pred,action_dist = self.actor_network.evaluate(state)
        log_prob_action = action_dist.log_prob(action_pred)
        ...
        # SAC Critic Update
        with torch.no_grad():
            next_action,next_action_dist = self.actor_network.evaluate(next_state)
            next_log_prob_action = next_action_dist.log_prob(next_action).squeeze()
            next_q_target_1 = self.critic_target_network_1(next_state, next_action).squeeze()
            next_q_target_2 = self.critic_target_network_2(next_state, next_action).squeeze()
        ...
        q1_val = self.critic_network_1(state, action).squeeze()
        q2_val = self.critic_network_2(state, action).squeeze()
