...
class CDS(nn.Module):
    def __init__(self, state_dim, hidden, action_dim, tau=None, target_entropy=None, temperature=None , batch_size = 256,task_num =3,task = None):
        super(CDS, self).__init__()

        self.actor_network = models.actor(state_dim + task_num, hidden, action_dim)
        self.critic_network_1 = models.critic(state_dim + action_dim + task_num, hidden)
        self.critic_target_network_1 = models.critic(state_dim + action_dim + task_num, hidden)
        self.critic_target_network_1.load_state_dict(self.critic_network_1.state_dict())
        self.critic_network_2 = models.critic(state_dim + action_dim + task_num, hidden)
        assert self.critic_network_1.parameters() != self.critic_network_2.parameters()
        self.critic_target_network_2 = models.critic(state_dim + action_dim + task_num, hidden)
        ...
        self.memory_2 = Buffer.Replay_buffer(self.batch_size)
    ...
    def train_net(self):
        ...
        if self.task == "run-forward" :
            task_state = np.array([[1.,0.,0.]])
            task_state = torch.from_numpy(np.repeat(task_state,self.batch_size,axis=0))
        elif self.task =="run-backward" :
        ...
        for i in range(self.epoch):
            ...
            state = torch.cat([state,task_state],axis=1)

            next_state = torch.cat(next_state)
            next_state = next_state.reshape(-1, self.state_dim)
            next_state = torch.cat([next_state,task_state],axis=1)
            ...
            random_action = torch.FloatTensor(q1_val.shape[0] * 10, action.shape[-1]).uniform_(-1, 1)
